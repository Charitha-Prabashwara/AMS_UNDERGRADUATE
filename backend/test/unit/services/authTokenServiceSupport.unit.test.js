...
describe('authTokenServiceSupport Complete Test Suite', () => {
    ...
    describe('generatePayload', () => {
        test('default priority', () => {
            const payload = authTokenServiceSupport.generatePayload('admin', 1, 1234567890);
        ...
        test('custom priority and other fields', () => {
            const payload = authTokenServiceSupport.generatePayload('student', 2, 987654321, 5, { foo: 'bar' });
            expect(payload).toEqual({
                userType: 'student',
                id: 2,
                priority: 5,
    ...
    describe('generateAccessToken', () => {
        test('should generate access token', () => {
            ...
            expect(jwt.sign).toHaveBeenCalledWith(payload, config.ACCESS_TOKEN_SECRET, { expiresIn: config.ACCESS_TOKEN_TTL });
        });
    });
    ...
    describe('verifyAccessToken', () => {
        test('valid token', () => {
            ...
            expect(jwt.verify).toHaveBeenCalledWith(token, config.ACCESS_TOKEN_SECRET);
        });
    ...
    describe('Concurrency / Stress Tests', () => {
        test('1000 concurrent payloads', async () => {
            const tasks = Array.from({ length: 1000 }, (_, i) =>
                Promise.resolve(authTokenServiceSupport.generatePayload('user', i, i * 1000))
            );
            const results = await Promise.all(tasks);
            expect(results[0]).toEqual({ userType: 'user', id: 0, priority: 0, expiresTimestamp: 0, other: {} });
            expect(results[999]).toEqual({ userType: 'user', id: 999, priority: 0, expiresTimestamp: 999000, other: {} });
        });

        test('1000 concurrent tokens', async () => {
            jwt.sign.mockImplementation(payload => `token-${payload.id}`);
            const payloads = Array.from({ length: 1000 }, (_, i) => ({ userType: 'user', id: i }));
            const tokens = await Promise.all(payloads.map(p => Promise.resolve(authTokenServiceSupport.generateAccessToken(p))));
    ...
    describe('Performance / Memory', () => {
        test('payload generation efficiency', async () => {
            ...
            console.log(`generatePayload 10,000 runs: ${(Number(end - start) / 1e6).toFixed(2)} ms`);
        });

        test('access token generation efficiency', async () => {
            jwt.sign.mockImplementation(p => `token-${p.id}`);
            const payloads = Array.from({ length: 10000 }, (_, i) => ({ userType: 'user', id: i }));
            const start = process.hrtime.bigint();
            await Promise.all(payloads.map(p => Promise.resolve(authTokenServiceSupport.generateAccessToken(p))));
            const end = process.hrtime.bigint();
            console.log(`generateAccessToken 10,000 runs: ${(Number(end - start) / 1e6).toFixed(2)} ms`);
        });
        ...
        test('memory usage', async () => {
            ...
            console.log(`Memory increase after 10,000 payloads: ${((finalMemory - initialMemory) / 1024 / 1024).toFixed(2)} MB`);
