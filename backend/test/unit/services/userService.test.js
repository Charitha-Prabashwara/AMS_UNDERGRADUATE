  ...
  test('should find each user from database', async () => {
   ...
   test('should create new Admin users using userService', async () => {
    ...
    test('should delete student by id suing userService', async () => {
      ...
      const deletedAdmin =await service.deleteUserById(userTypes.USER_ADMIN,admin.id);
      const deletedDepartmentHead =await service.deleteUserById(userTypes.USER_DEPARTMENT, departmentHead.id);
      const deletedLecturer =await service.deleteUserById(userTypes.USER_LECTURER, lecturer.id);
      const deletedStudent =await service.deleteUserById(userTypes.USER_STUDENT, student.id);
      ...
      expect(deletedDepartmentHead.registration_id).toBe(departmentHeadData.registration_id)
      ...
      expect(deletedDepartmentHead.address).toStrictEqual(departmentHeadData.address)
...
test('should update Admin by id suing userService', async () => {
   const dataGen = async()=>{
      ...
      const updatedAdmin =await service.findByIdAndUpdate(userTypes.USER_ADMIN,admin);
      const updatedDepartmentHead =await service.findByIdAndUpdate(userTypes.USER_DEPARTMENT, departmentHead);
      const updatedLecturer =await service.findByIdAndUpdate(userTypes.USER_LECTURER, lecturer);
      const updatedStudent =await service.findByIdAndUpdate(userTypes.USER_STUDENT, student);
