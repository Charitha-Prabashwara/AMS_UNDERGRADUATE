...
describe('Auth Service - Admin Logout (Integration)', () => {
    ...
    test('should logout successfully with real ObjectId', async () => {
        // login first
        const loginResponse = await service.login(userTypes.USER_ADMIN, admin.email, password);
        expect(loginResponse.tokens.access).toBeDefined();
        expect(loginResponse.cookie).toBeDefined();

        // logout using real ObjectId
        const logoutResponse = await service.logOut(userTypes.USER_ADMIN, new Types.ObjectId(admin.id));
        ...
        const updatedUser = await new UserService().getUserById(userTypes.USER_ADMIN, admin.id);
    ...
    test('should fail logout if user does not exist', async () => {
        ...
        await expect(service.logOut(userTypes.USER_ADMIN, fakeId))
            .rejects
    ...
    test('should fail logout if user is suspended', async () => {
        ...
        await expect(service.logOut(userTypes.USER_ADMIN, new Types.ObjectId(admin.id)))
            .rejects
            .toThrow('Logout failed');
