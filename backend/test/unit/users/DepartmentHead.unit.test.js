...
afterAll(async () => {
  ...
  test('Should create a new Department-Head successfully', async () => {
    ...
    const compare = await PasswordHashService.verifyPassword(defaultPassword, createDepartmentHead.password);
  ...
  test('Should delete Department-Head by ID (deleteById)', async () => {
    ...
    const findDeleted = await new DepartmentHead({ id: newDepartmentHead.id }).find();
  ...
  test('Should handle findById error gracefully', async () => {
    const badDepartmentHead = new DepartmentHead();
    badDepartmentHead.findById = jest.fn().mockRejectedValue(new Error('Find failed'));
    await expect(badDepartmentHead.findById('invalid')).rejects.toThrow('Find failed');
  });
