...
const {Student} = require('../../../src/classes/USERS');
...
afterAll(async () => {
  ...
  test('Should create a new student successfully', async () => {
    ...
    const compare = await PasswordHashService.verifyPassword(defaultPassword, createStudent.password);
  ...
  test('Should not allow modification of _type in student or studentBuilder', () => {
    ...
    //expect(Object.getOwnPropertyDescriptor(student, '_type').writable).toBe(false);
